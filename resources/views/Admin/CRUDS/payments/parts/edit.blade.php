...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('payments.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        <div class=" row my-4">
            @foreach($services as $service)
                <div class="col-md-4 p-1">
                    <span class="form-check form-switch  " @if( app()->getLocale()=='en') style="border:1px solid #F3F3F9;padding: 10px; padding-left: 40px;border-radius: 4px;" @else  style="border:1px solid #F3F3F9;padding: 10px; padding-right: 40px;border-radius: 4px;" @endif>
                      <input class="form-check-input  checkbox" type="checkbox" @foreach(\App\Models\PaymentService::where('payment_id',$row->id)->get() as $pivot)  @if($pivot->service_id==$service->id) checked @endif @endforeach name="services[]" value="{{$service->id}}"
